...
@Mapper(uses = { OrderItemMapper.class, DeliveryMapper.class }, componentModel = "spring")
public interface OrderMapper {
    ...
    @Mappings({
            @Mapping(target = "orderItemSimpleList", source = "orderDocument.orderItems",
                    qualifiedByName = "toOrderItemSimpleListForOrderSimple"),
    ...
    @Named("toOrderItemSimpleListForOrderSimple")
    default List<OrderItemSimple> toOrderItemSimpleListForOrderSimple(List<OrderDocument.OrderItem> orderItems,
                                                        @Context OrderItemMapper orderItemMapper,
                                                        List<ItemDocument> itemDocuments) {
        return orderItemMapper.toOrderItemSimpleList(orderItems, itemDocuments);
    }

    List<OrderSimple> toOrderSimpleList(List<OrderDocument> orderDocuments, List<List<ItemDocument>> itemDocumentsList, List<DeliveryStatus> deliveryStatuses);
